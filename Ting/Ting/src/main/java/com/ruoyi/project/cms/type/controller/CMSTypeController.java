...
public class CMSTypeController extends BaseController {
    ...
    public TableDataInfo list(CMSType type){
        //分页处理
        startPage();
        //查询，通过服务层调用mapper，查询数据
        List<CMSType> list = icmsTypeService.selectCMSTypeList(type);
    ...
    public String checkTypeNameUnique(CMSType type){
        return icmsTypeService.checkTypeNameUnique(type);
    }
    ...
    public AjaxResult add(@Validated CMSType type){
        if(CMSTypeConst.TYPE_NAME_NOT_UNIQUE.equals(icmsTypeService.checkTypeNameUnique(type))){
            // 公告类型名称不可用。
        ...
        return toAjax(icmsTypeService.addCMSType(type));
    ...
    {
        model.addAttribute("type", icmsTypeService.selectCMSTypeById(typeId));
        return prefix + "/edit";
    }
    ...
    public AjaxResult editSave(@Validated CMSType type)
    {
        if (CMSTypeConst.TYPE_NAME_NOT_UNIQUE.equals(icmsTypeService.checkTypeNameUnique(type)))
        {
            return error("修改公告类型名称'" + type.getTypeName() + "'失败，公告类型名称已存在");
        }

        return toAjax(icmsTypeService.updateCMSType(type));
    }
    ...
    public AjaxResult remove(String ids){
        try{
            return toAjax(icmsTypeService.removeCMSTypeByIds(ids));
        }catch(Exception e){
